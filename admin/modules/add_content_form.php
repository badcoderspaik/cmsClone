...
$form_connector_result = $form_connector->select("SELECT name FROM menu", false);
...
if($_POST['btn_add_post']){
    if($book_title && $author && $year && $description && $cover && $book) {
        ...
        $form_connector->insert("INSERT INTO `books` (`title`, `author`, `year`, `text`, `image`, `category_id`) VALUES (\"$book_title\", \"$author\",\"$year\", \"$description\", \"$cover\", '1')");
